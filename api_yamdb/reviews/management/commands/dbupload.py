...
class Command(BaseCommand):
    ...
    def prep_values(self, data: dict):
        ...
        Экранируется знак (')"""
        return ", ".join(
            "'" + str(value).replace("'", "''") + "'" for
            value in data.values()
        )

    def prep_name_columns(self, name_columns_from_db: list, row: dict):
    ...
    def save_avg_rating(self, connect: sqlite3.Connection,
        ...
        sql_get_titles_id = """
            ...
            FROM reviews_title
            """
        titles_id = cursor.execute(sql_get_titles_id).fetchall()
        ...
        for title_id in titles_id:
            sql_get_avg_score = f"""
                ...
                """
            title_avg_score = cursor.execute(sql_get_avg_score).fetchall()
    ...
    def handle(self, *args, **options):
        ...
        for file_name, table_name in self.TABLE_NAMES.items():
            ...
            self.stdout.write(f"Заполнение таблицы {table_name}........",
                              ending="")
            # Открываем файл
            with open(
                "static/data/" + file_name, newline="", encoding="UTF8"
            ) as csvfile:
                ...
                for row in reader:
                    ...
                    prep_name_col = self.prep_name_columns(
                        self.get_name_columns_from_db(cur, table_name, 0), row
                    )
                    ...
                    if file_name == "users.csv":
                        ...
                        values = (
                            values
                            + ", "
                            + self.get_value_from_dict(self.USER_DEFAULT_COLUMN)
                        )
                    try:
                        ...
                        self.stdout.write(f">>> ОШИБКА. " f"Error: {error}")
                    con.commit()
